...
public class SmtpEmailSender(IConfiguration configuration, ILogger<SmtpEmailSender> logger) : IEmailSender<User>, ISmtpEmailSender
{
    ...
    {
        const string subject = "Reset Your Password - AiCV";
        const string heading = "Password Reset Request";
        const string messageBody = "<p>Hello,</p><p>We received a request to reset the password for your account. Click the button below to set a new password. This link will expire shortly.</p>";
    ...
    {
        const string subject = "Confirm Your Email - AiCV";
        const string heading = "Confirm Your Email Address";
        const string messageBody = "<p>Hello,</p><p>Thank you for signing up for AiCV! Please click the button below to verify your email address and activate your account.</p>";
    ...
    {
        const string subject = "Your Password Reset Code - AiCV";
        const string heading = "Password Reset Code";
        string messageBody = $"<p>Hello,</p><p>We received a request to reset your password. Use the following code to complete the request:</p><p style=\"font-size: 32px; font-weight: 800; color: #1e293b; letter-spacing: 4px; text-align: center; margin: 24px 0; background-color: #f1f5f9; padding: 16px; border-radius: 8px; font-family: monospace;\">{resetCode}</p>";
        var body = GetEmailHtmlLayout(subject, heading, messageBody);
        await SendEmailAsync(email, subject, body);
    }

    public async Task SendAutomationSummaryAsync(string to, string subject, string heading, string bodyHtml)
    {
        var body = GetEmailHtmlLayout(subject, heading, bodyHtml);
        await SendEmailAsync(to, subject, body);
    }

    private static string GetEmailHtmlLayout(string title, string heading, string messageBody, string? actionText = null, string? actionUrl = null)
    {
        var buttonHtml = "";
